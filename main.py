...
def run_all_tests_for_function(f, title, fmt, exclude=None):
    print(title)
    cases: list[tuple[str, float]] = []
    for cls in set(code_classes).difference(exclude or set()):
        cases.append(f(cls))
...
def test_creation_speeds():
    return run_all_tests_for_function(
        run_create,
        "creation speeds",
        "{}: {} - {:.0f} ns",
        exclude={},
